...
| Browser profiles (cookies, localStorage)              | `<data>/browser-sessions/<key>/`             | No (Chromium profile dir)                             | Stay-logged-in sessions are real Chromium profiles: whoever can read these files can ride your logins. Keep the host trusted; delete a session to wipe its profile and forget the login. Reads need no approval; every click/type needs a human-approved approval request. |
