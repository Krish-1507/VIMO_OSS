...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Give the auth store a tick to persist before navigating — prevents
      // the AppRoutes guard from seeing stale isAuthenticated and bouncing
      // back to /login (the "glitch" after login).
      await new Promise((r) => setTimeout(r, 50));
      navigate('/dashboard', { replace: true });
    } catch (err: any) {
      const msg = err?.response?.data?.message || err?.response?.data?.error || 'Incorrect PIN';
      setError(msg);
      setShake(true);
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 dark:bg-slate-900 p-4 animate-in fade-in duration-700">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {busy ? <span className="h-4 w-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : null}
